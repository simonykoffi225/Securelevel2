...
if (isset($recherche)) {
    if (($lieu != 0) && ($annee != 0)) {
        for ($mois = 1; $mois <= 12; $mois++) {
            @$info = $db->prepare("SELECT COUNT(*) AS nbAccident FROM incendie INNER JOIN ville ON lieu = id WHERE MONTH(dateEvenement) = $mois AND (lieu =$lieu) OR (YEAR(dateEvenement) = $annee)");
            @$info->execute();
                    ...
                    <div class="section_centrale">
                        ...
                        <div class="formulaire">
                            <div style="text-align:center; margin-top: 40px;">
                                <h3>Recensement Graph Incendie</h3>
                                <p style="font-weight:bold; font-size: 15px;"> Se Graphe Nous Permet De Voir <br> Les Statistiques Des Différents Incendie Dans Les Régions De La CI</p>
                            </div>
                            <form action="graph_incendie.php" method="GET">
                                <div class="chartCard">
                                    <div class="chartBox">
                                ...
                                <?php include("datas_incendie.php"); ?>
